...
/**
 * Добавляет меню AI-Редактор.
 * Важно: не используем `onOpen()` напрямую, чтобы не конфликтовать с существующим меню
 * в вашем проекте (когда в Apps Script уже есть другой `onOpen`).
 */
function addAiEditorMenu_() {
...
function buildPrompt_({
  ...
  const systemPrompt =
    ...
    `---\n` +
    `### **Формат вывода:**\n` +
    `Верни *только готовый редакторский комментарий* на русском в формате буллитов.\n` +
    `Формат: 3–6 строк, каждая строка начинается с "- " (например: - ...).\n` +
    `Без заголовков и без нумерации. Внутри буллета можно вставлять пример в кавычках «…».\n\n` +
    `---\n` +
